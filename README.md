...
Browse and install approved MES Android apps, read guides, and keep your apps up to date. Android 9 or newer is required.

## Download and release status

**[Download MES Store 1.2.25](https://github.com/huangxuewu/mes-app-releases/releases/download/mes-store-v1.2.25/mes-store-1.2.25.apk)**

Release snapshot: September 23, 2026. The [live v2 catalog](https://mes.dhmsc.com/store/catalog-v2.json) is authoritative; [catalog-v2.json](catalog-v2.json) is its GitHub mirror.

| App | Version | Version code | Installation |
| --- | --- | --- | --- |
| Dispatcher | 1.1.4 | 8 | Available in MES Store |
| MES Radio | 1.0.8 | 12 | Uninstall older app first; local data is deleted |
| MES Run Beta | 0.1.0-beta.25 | 36 | Uninstall older app first; local data is deleted |
| MES Store | 1.2.25 | 31 | Available in MES Store |
| SignPad | 1.6.4 | 14 | Available in MES Store |
| Time Punch | 1.2.2 | 9 | Available in MES Store |
| Upkeep Beta | 0.1.0-beta.1 | 1 | Available in MES Store |

### Signing migrations

- **MES Store older than 1.2.24:** download the current Store APK directly and install it over the existing Store. Do not uninstall Store. It retains its Android signing identity and app data while using the replacement v2 release-approval authority. The old catalog is retained for older Stores.
- **MES Radio 1.0.8 and MES Run Beta 25:** these releases use owner-approved replacement Android signing keys and retain their package IDs. They cannot install over older versions. Finish or review pending work, uninstall the old app yourself, then install the new version and sign in again. Uninstalling deletes local settings, drafts, pending actions and recordings; server-side records are not deleted by uninstalling. The Store does not bypass Android signature checks.
- **Dispatcher, SignPad and Time Punch:** install updates over the existing apps to preserve their data and authorization.

MES Run Beta 25 connects to the production MES service and requires an authorized supervisor PIN. Upkeep Beta is a device-only procedure sandbox: it uses in-memory sample data, makes no server connections or real purchases, and resets business data when its process restarts.

### Install an app

Open MES Store and refresh the Apps list. Select an app, download it, then choose **Install now**. Enable Android's **Allow from this source** permission for MES Store when prompted and complete Android's confirmation. No physical devices were automatically updated for this release.

This public repository contains only distribution catalogs, documentation and release assets. Source and signing-key backups remain in private app repositories. Public downloads do not grant MES company access.
...
The tutorial index response is limited to 2 MiB and 100 articles. Each article supports 1¨C200 pages, 1¨C200 sections, and 1¨C20 steps per section. PDFs are limited to 25 MiB. Page references must fall within the declared page count; text lengths, IDs, and hashes are validated. Initial PDF URLs must be HTTPS release download URLs in this repository, ending in `.pdf`, without query strings or fragments.
